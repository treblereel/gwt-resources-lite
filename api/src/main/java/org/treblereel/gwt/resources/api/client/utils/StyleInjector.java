 ...
 */
package org.treblereel.gwt.resources.api.client.utils;
...
public class StyleInjector {
  ...
  private static final JsArray<String> toInject = JsArray.of();
  private static final JsArray<String> toInjectAtEnd = JsArray.of();
  private static final JsArray<String> toInjectAtStart = JsArray.of();

  private static Callback flusher =
      new Callback() {
        public void execute() {
          if (needsInjection) {
            flush(null);
  ...
  private static HTMLStyleElement flush(JsArray<String> which) {
  ...
  private static void schedule() {
    if (!needsInjection) {
      needsInjection = true;
      DomGlobal.setTimeout(p0 -> flusher.execute(), 0);
    }
  }

  private static HTMLStyleElement injectStyleSheet(String contents) {
    HTMLStyleElement style = createElement(contents);
    getHead().appendChild(style);
    return style;
  }

  private static HTMLStyleElement injectStyleSheetAtEnd(String contents) {
    return injectStyleSheet(contents);
  }

  private static HTMLStyleElement injectStyleSheetAtStart(String contents) {
    HTMLStyleElement style = createElement(contents);
